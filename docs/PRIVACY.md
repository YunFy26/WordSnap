...
| 鼠标点击事件 | 仅在本机判断点击是否发生在翻译卡片外 | 翻译完成后自动隐藏卡片；不记录点击位置或点击内容 |
...
macOS 上，WordSnap 会在应用运行期间监听鼠标按下事件，用于在翻译完成后判断用户是否点击了卡片之外的区域。它不读取点击目标的内容，不保存或发送点击坐标，也不会在翻译仍处于加载状态时隐藏卡片。
